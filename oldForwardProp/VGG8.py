...
from paramParser import parse_params_file
from paramParser import input 
...
class VGG8(nn.Module):
    ...
    def forward(self, x):
        x = self.cnnlayer00(x)
        print("After cnnlayer00:", x.shape)
        x = self.act00(x)
        x = self.cnnlayer01(x)
        x = self.act01(x)
        x = self.maxpool0(x)
...
def assign_weights(model, layer_weights, layer_biases):
    for name, layer in model.named_modules():
        if isinstance(layer, (nn.Conv1d, nn.Linear)):
            if name in layer_weights:
                # Reshape before assigning
                weight_shape = layer.weight.data.shape
                layer.weight.data = torch.tensor(layer_weights[name].reshape(weight_shape), dtype=torch.float32)
            if name in layer_biases:
                layer.bias.data = torch.tensor(layer_biases[name], dtype=torch.float32)

def main():
    param_path = "params.h"

    # Parse weights and biases from the header
    ...
    # Assign weights
    layer_weights = {k: v['weight'] for k, v in layer_params.items()}
    layer_biases = {k: v['bias'] for k, v in layer_params.items()}
    assign_weights(model, layer_weights, layer_biases)

    # Run dummy inference (or load your real input here)
    input_np = np.array(input, dtype=np.float32)  # Assuming input is a numpy array
    input_tensor = torch.from_numpy(input_np).view(1,2,1024)
    output = model(input_tensor)

    # Show predicted class
    pred = torch.argmax(output, dim=1)
    print(f"Predicted class index: {pred.item()} -> {model.classes[pred.item()]}")
